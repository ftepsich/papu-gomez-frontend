...
class Paginacion extends Component {
  ...
  generalChangePage = (element) => {
    ...
    for (
      let i = (iPage - 1) * this.state.elemxpage, j = 0;
      j < this.state.elemxpage && i < rows.length;
      j++, i++
    ) {
  ...
  changePageNext = () => {
    let pageActiveNow = document
      ...
      .querySelector(".active");
    if (
      pageActiveNow.dataset.key !==
      document.getElementsByClassName("pagination-list-item").length
    ) {
  ...
  renderInfoUI = (draw = false, row = null) => {
    if (this.props.info) {
      return (
        <div className="pagination-col-item">
          {draw ? row[this.props.info] : ""}
        </div>
      );
    }
  };
    ...
    return (
      <div className="pagination">
        <div className="pagination-nav">
          <div
            className="pagination-list-item-prev"
          ...
          <div className="pagination-list">
            {(() => {
              ...
              paginationItem.push(
                <div
                  className="pagination-list-item active"
                  data-key="1"
          ...
          <div
            className="pagination-list-item-next"
            data-key="next"
        ...
        {this.props.rbody.map((row, key) => (
          ...
          >
            {row.map((ecol, kitem) =>
              `${kitem}` !== this.props.info &&
              `${kitem}` !== this.props.delete &&
              `${kitem}` !== this.props.edit ? (
                <div
                  className={`pagination-${this.state.ncols}-col`}
                  key={kitem}
                >
                  {ecol !== "null" ? ecol : "-"}
                </div>
